...
test("server-renders the Rakmaro landing page", async () => {
  ...
  assert.match(html, /Ils nous font confiance pour leur installation aux Émirats|They trust us for their UAE relocation/);
  assert.match(html, /Consultant indépendant|Independent consultant/);
  assert.match(html, /Compte professionnel préparé dès la création|Business account prepared from company setup/);
  assert.match(html, /L’ouverture et les délais restent soumis à la validation de la banque|subject to bank approval/);
  assert.match(html, /Parler sur WhatsApp|Talk on WhatsApp/);
  assert.match(html, /Obtenez une première estimation en 60 secondes|Get a first estimate in 60 seconds/);
  ...
  assert.match(html, /environ 7 jours|about 7 days/);
  assert.match(html, /Agent RAKEZ autorisé|Authorized RAKEZ Agent/);
  assert.doesNotMatch(html, /Étude humaine nécessaire/);
  assert.doesNotMatch(html, /Structure temporaire|témoignages fictifs|placeholder testimonials|Camille Laurent|Yanis Rahmani|Sarah Benali/);
  assert.doesNotMatch(html, /Ouverture de compte bancaire sous 7 jours|Business account opening within 7 days|Budget affiché après sélection/);
